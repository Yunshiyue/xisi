...
public class SpeakingDialog : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.Q) || platformJudge.IsQ()) && index == textList.Count)//全部文本输入完毕后
        {
            platformJudge.ClearQ();
        ...
        if (Input.GetKeyDown(KeyCode.Q) || platformJudge.IsQ())
        {
            platformJudge.ClearQ();
    ...
    IEnumerator setTextUI()//进行打字的操作
    {
        ...
        switch (textList[index].Trim())//判断说话的人是主角还是NPC，B是主角说的，A是NPC说的
        {
            case "B":
