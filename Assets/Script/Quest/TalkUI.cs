...
public class TalkUI : MonoBehaviour
{
    ...
    void Update()
    {
        if(Button_trigger.activeSelf && (Input.GetKeyDown(KeyCode.Q) || platformJudge.IsQ()) && !isEndDialog1)//如果对话一没进行过，则进行对话一 
        {
            Dialog1.SetActive(true);
        }
        if (Button_trigger.activeSelf && (Input.GetKeyDown(KeyCode.Q) || platformJudge.IsQ())&& isEndDialog1 && !isEndDialog2)//接受任务但是没完成则进行这个对话
        {
            Dialog2.SetActive(true);
        }
        if(Button_trigger.activeSelf && (Input.GetKeyDown(KeyCode.Q) || platformJudge.IsQ()) && isEndDialog1 && isEndDialog2 && !isEndDialog3)//完成任务后进行这个对话
        {
